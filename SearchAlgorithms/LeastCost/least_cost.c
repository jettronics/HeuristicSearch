...
int applicable( char name1, char name2, float *effort )
{
   int i;
   int applicable = 0;
   ...
   for( i=0; i<size; i++ )
   {
      if( ((street_list[i].start == name1) || (street_list[i].end == name1))
          &&
          ((street_list[i].start == name2) || (street_list[i].end == name2)) )
      {
         applicable = 1;
         *effort = cost( name1, name2 );
         return 1;
      }
   }
   return 0;
...
int find_children( char start, int index, cost_t *p_cost_list )
{
   int i;
   int n = 0;
   float effort;
...
/* route determination */
void least_cost_search( char start, char destination, int index, float actual_cost )
{
   if( start != destination )
   {
      ...
      if( found > 0 )
      {
         ...
         for( n=0; n<found; n++ )
         {
            ...
            least_cost_search( cost_list[n].name, destination, index+1, new_cost);
         }
      }
   }
   else
...
void main( void )
{
   ...
   result_list[0] = 'a';
   least_cost_search( result_list[0], 'e', 1, 0.0);
